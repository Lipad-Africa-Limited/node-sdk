...
class Lipad {
  ...
  async getAccessToken() {
    ...
    const postData = new URLSearchParams(authData).toString();
    const apiUrl = `${
      CHECKOUT_BASE_URL[this.environment]
  ...
  async checkCheckoutStatus(merchant_transaction_id, access_token) {
    const apiUrl = `${
      CHECKOUT_BASE_URL[this.environment]
    }/checkout/request/status?merchant_transaction_id=${merchant_transaction_id}`;
  ...
  async getCheckoutStatus(merchant_transaction_id) {
    try {
      ...
      return await this.checkCheckoutStatus(
        merchant_transaction_id,
        access_token
      );
    } catch (error) {
  ...
  async DirectCharge(payload) {
    try {
      const baseUrl = `${
        DIRECT_CHARGE_BASE_URL[this.environment]
      }?payment_method=`;
      ...
      const endpoint =
        paymentMethodMap[paymentPayload.payment_method_code] || "";
      ...
      if (endpoint) {
        ...
        throw new Error(
          "Invalid payment method code: " + paymentPayload.payment_method_code
        );
      }
    } catch (error) {
      console.error("Error:", error.message);
  ...
  buildPaymentPayload(payload) {
    ...
    return payload.payment_method_code === "MPESA_KEN"
      ? mpesaPayload
      : airtelPayload;
  }
  ...
  async getChargeRequestStatus(chargeRequestId) {
    try {
      ...
      const baseUrl = `${
        DIRECT_CHARGE_BASE_URL[this.environment]
      }/transaction/${chargeRequestId}/status`;
      const headers = {
        "x-access-token": accessToken,
        "Content-Type": "application/json",
      };
      ...
      if (response.status === 200) {
        ...
        console.error(
          `Failed to make GET request. Response code: ${response.status}`
        );
        throw new Error(
          `Failed to make GET request. Response code: ${response.status}`
        );
